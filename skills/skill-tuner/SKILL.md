---
name: skill-tuner
description: "Autonomous SKILL.md optimizer: 9-dim rubric scoring, paired same-judge majority vote, 3-round hill-climbing on a git branch with revert rollback, test-prompt validation, human checkpoints. Use when user says 优化 skill, skill 调优, skill 评分, 自动优化, skill review, 帮我改改 skill, tune skill."
---

# Skill Tuner

> Fork of [alchaincyf/darwin-skill](https://github.com/alchaincyf/darwin-skill) (MIT, upstream last synced 2026-07-27). 本地化改动：无成果卡、工件落 XDG、不增重规则。
...
`test-prompts.json` 与 `results.tsv` 不落 skill 目录，统一落：

```
~/.local/share/agent-workfiles/<repo>/reports/skill-tuner/<skill-name>/
```

`<repo>` = 被优化 skill 所在仓的主 worktree 目录名：在该仓内执行 `git rev-parse --path-format=absolute --git-common-dir`，结果 basename 为 `.git` 时取其父目录 basename（任意 worktree 中一致）。不在 git 仓库时，与异常表「不在 git 仓库」一并向用户确认 `<repo>` 取值。下文 `<workfiles>` 均指此目录。
...
```
1. 确认优化范围：
   - 全部skills → 扫描 ~/.agents/skills/*/SKILL.md（本机共享 skills 目录；Claude/Codex/Kimi 各自的 skills 目录是指向它的 symlink）
   - 指定skills → 用户指定列表
2. 创建 git 分支：auto-optimize/YYYYMMDD-HHMM
3. 初始化 `<workfiles>/results.tsv`（如不存在）
...
for each skill:
  ...
  2. 设计2-3个测试prompt，覆盖：
     - 最典型的使用场景（happy path）
     - 一个稍复杂或有歧义的场景
  3. 保存到 `<workfiles>/test-prompts.json`：
     [
...
```
文件位置：`<workfiles>/results.tsv`（见「工件落点」）
...
| 候选版比现版更重 | 字节数、步骤数、子代理调用数任一增加 | 必须在汇报里写明是哪个行为测试要求的；成对评审打平时选更轻的一版 |
| test-prompts.json 已存在 | 文件已在 `<workfiles>` 目录 | 默认复用并展示，问用户「复用 / 重写 / 追加」三选一 |
...
## skill-tuner 操作反例黑名单（dim9 应用：skill-tuner 自己优化时不要做的事）
...
4. **不增重** — 候选版字节数、步骤数、子代理调用数任一增加，必须写明是哪个行为测试要求的；成对评审打平时选更轻的一版
5. **尊重花叔风格** — 中文为主、简洁为上
6. **可回滚** — 所有改动在git分支上，用git revert而非reset --hard
7. **评分独立性** — 效果维度必须用子agent或至少干跑验证，不能在同一上下文里「改完直接评」
8. **Runtime 中立性** — skill 必须能在 Claude Code、Codex、Cursor、OpenClaw、Hermes 等任何 skills-compatible runtime 中正常运行。除非 skill 名明确绑定单一 runtime（如 `xxx-codex`、`huashu-slides-codex`），任何「在 Claude Code 里」「Claude Code skill」「单一 badge 钉死」「安装命令只给 `~/.agents/skills/` 一种路径」都视为 gate 不通过，须在 P0 优先修复（详见「Runtime 适配性审查」章节）
...
- **autoresearch**：[github.com/karpathy/autoresearch](https://github.com/karpathy/autoresearch)，上游 darwin-skill 1.0 的原始灵感。
