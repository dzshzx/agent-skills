...
their trust and permission scope from cwd, and none of them takes it from the prompt text.
...
```bash
BRIEF=$(mktemp) OUT=$(mktemp)
cat > "$BRIEF" <<'EOF'
...the brief...
EOF
codex exec --skip-git-repo-check --json -o "$OUT" "$(cat "$BRIEF")" </dev/null
```

The quoted `"$(cat …)"` keeps the whole brief one argv entry; the same form carries the prompt
for all three CLIs. Building the command by concatenating prompt text into it is how a
...
Kimi as it comes. When you want a report rather than edits, say so in the brief; that is how the
...
command timeout cuts them off mid-task and the work is lost. One invocation per task the user
named — each reloads the delegate's whole instruction layer and may take many model turns —
split only where a later dispatch would restart from a saved result.
