...
| [`skill-tuner`](skills/skill-tuner/SKILL.md) | Autonomous SKILL.md optimizer forked from [alchaincyf/darwin-skill](https://github.com/alchaincyf/darwin-skill) (MIT): 9-dimension rubric, paired same-judge majority vote, 3-round hill-climbing on a git branch with `git revert` rollback, test-prompt validation, human checkpoints. Local changes: no result card, artifacts under the XDG agent-workfiles directory, no-weight-gain rule. |
