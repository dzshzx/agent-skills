...
| [`cross-agent-delegation`](skills/cross-agent-delegation/SKILL.md) | Hands a task to a different vendor's coding agent CLI (Claude Code, Codex, Kimi Code) as a headless subprocess: the minimal correct invocation per CLI, the silent-failure gotchas each one hides (Kimi's non-existent `-r` resume flag, Claude reporting `subtype:"success"` on a failed run, Codex blocking on a non-TTY stdin), a role-to-permission table, and the brief and handoff contracts a context-free subprocess needs. |
