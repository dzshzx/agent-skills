...
| [`cross-agent-delegation`](skills/cross-agent-delegation/SKILL.md) | Hands a task to a different vendor's coding agent CLI (Claude Code, Codex, Kimi Code) as a headless subprocess: per-CLI invocation contracts, a role-to-permission table, and the brief and handoff rules a context-free delegate needs. Ships `evals/live-check.sh`: a default tier that re-checks every flag and rejection with no model calls, plus `--smoke` for the JSON fields and permission behaviour `--help` never confesses. |
